...
_base_addons() {
  ...
  if [ "$INSTALLTYPE" == "desktop" ]; then
    ...
    printf "Not sure what this is, panic"
...
_install_ssd() {
  ...
  if [ $user_confirmation == "0" ]; then
    ...
    if [[ ${datadevicename:5:4} = "nvme" ]]; then
  ...
  while [ $userinputdone -ne 0 ]; do
    ...
    if [ "$INSTALLTYPE" == "server" ]; then
      ...
      while [ $finished -ne 0 ]; do
        ...
        if [ "$SSHPORT" -eq "$SSHPORT" ]; then # 2>/dev/null
          if [ $SSHPORT -lt 8000 ] || [ $SSHPORT -gt 48000 ]; then
      ...
      while [ $finished -ne 0 ]; do
        ...
        if [ "$lasttriad" -eq "$lasttriad" ]; then # 2>/dev/null
          if [ $lasttriad -lt 150 ] || [ $lasttriad -gt 250 ]; then
...
_server_setup() {
  ...
  if [[ ${ETHERNETDEVICE:0:3} == "eth" ]]; then
    rm /etc/systemd/network/eth*
  fi

  if [[ ${ETHERNETDEVICE:0:3} == "enp" ]]; then
